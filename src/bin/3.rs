    ...
    for line in sup.lines {
        ...
        let priochr:Vec<char> = {
            if sup.args.part_two {
                ...
                {
                    nelves = 0;
                    let isect: HashSet<char> = HashSet::from_iter(prevdataset.into_iter().filter(|ch| prevprevdataset.contains(ch)));
                    let priochr: Vec<char> = Vec::from_iter(dataset.into_iter().filter(|ch| isect.contains(ch)));
                ...
                Vec::from_iter(line[linelen/2..linelen].chars().filter(|ch| dataset.contains(ch)))
            }
        };
