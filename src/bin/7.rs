...
impl DirTree {
    fn add_file(&mut self, dir: usize, name: &str, filesize: u32) {
        let mut mydir = dir;
    ...
    fn add_subdir(&mut self, dir: usize, name: &str) {
    ...
    fn find_subdir(&mut self, dir: usize, name: &str) -> Option<usize> {
    ...
    fn find_parent(&self, dir: usize) -> Option<usize> {
        let dir = &self.arena[dir];
        dir.parent
    }

    fn find_upto_size(&self, dir: usize, dirsize: u32) -> u32 {
        let dir = &self.arena[dir];
        let totalsize = dir
            .subdirs
            .iter()
            .map(|kv| self.find_upto_size(*kv.1, dirsize))
            .sum();
        if dir.size <= dirsize {
            totalsize + dir.size
        } else {
...
fn main() -> io::Result<()> {
    ...
    if sup.args.part_two {
        ...
        println!(
            "Free space: {}  Amount to delete: {}",
            free_space, amount_to_delete
        );
        let mut smallvec = tree
            .arena
            .iter()
            .filter_map(|d| {
